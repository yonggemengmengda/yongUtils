...
YongUtils 是一个面向前端开发者的 VS Code 效率插件，聚焦代码整理、翻译与 i18n、类型生成、AST 洞察和常用开发工具。

如果你经常在项目里处理 import、中文文案、命名、类型定义或 Vue 3 组件结构，YongUtils 可以把这些高频操作收拢到一个更顺手的工作流里。
...
- 按照副作用导入、Node.js 内置模块、第三方依赖、alias 模块、相对路径模块分组
- 支持 `import type`、副作用 import 固定置顶、按需移除未使用 import
- 支持读取 `tsconfig/jsconfig` 的 `paths` alias 规则
- 支持 Code Action 与保存时自动整理
- 保持代码整洁，提高可读性

**使用方式**：右键编辑器 → "Sort Imports"，或在命令面板执行 "打开 YongUtils 工具面板"，也可以开启保存时自动整理
...
### 6. i18n 文案提取助手
将选中的中文文案快速转换为 i18n key，并生成可直接使用的替换片段：
- 自动根据当前文件名生成 key 前缀
- 自动生成英文 key 名称与英文翻译
- 支持 `t('key')`、`{{ t('key') }}`、`$t('key')`、`this.$t('key')` 等替换形式
- 自动将 `zh-CN / en-US` 语言包条目复制到剪贴板

**使用方式**：选中文案或字符串字面量 → 右键编辑器 → "提取为i18n文案"

### 7. Webview 多功能工具面板
内置图形化工具面板，集中提供 AST、正则、图片处理和编码工具：

#### 🔍 AST 代码解析预览
**实时 AST 分析**：深度解析 Vue 3/HTML/JavaScript/TypeScript 文件的抽象语法树！

**功能特性**：
- **多语言支持**：Vue 3 SFC (.vue)、HTML (.html)、JavaScript (.js/.jsx)、TypeScript (.ts/.tsx)
...
- **高级搜索**：
  ...
  - 支持多关键词空格分隔搜索（需全部匹配）
  - 实时高亮匹配节点
- **结构洞察**：针对 Vue 3 文件提供模板组件、脚本结构、样式分析等专门视图

**兼容范围说明**：
- Vue AST 面板的深度摘要、模板联动、组合式 API 洞察目前只面向 Vue 3 SFC
- 推荐搭配 `<script setup>` 和 Composition API 使用
- Vue 2 / Options API 不在当前兼容承诺范围内
...
#### AI 翻译配置
要使用 AI 翻译功能，需要先在工具面板的“翻译管理”里配置模型信息：

1. 执行命令 `打开 YongUtils 工具面板`
2. 进入“翻译管理”
3. 填写 `API Key`、`Base URL` 和 `模型`
4. 保存后即可用于翻译、命名和 i18n 提取

内置预设包括 OpenAI、通义千问、DeepSeek、SiliconFlow，也支持自定义兼容 OpenAI 的接口。
...
- **本地优先**: AST、Import 排序、类型生成、正则测试、图片处理等功能都在本地完成
- **翻译走你配置的模型服务**: 只有翻译、命名和 i18n 英文生成会请求你在“翻译管理”中配置的接口
- **API Key 安全**: API Key 通过 VS Code Secret Storage 保存，不会硬编码到项目里
